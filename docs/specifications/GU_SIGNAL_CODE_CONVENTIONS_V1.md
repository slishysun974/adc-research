...
这里的仿射关系是 **v1 静态行为约简**：代码中第二级 flash 输入由第一级主
residue 乘比例并加 offset 得到。论文的第一级主、辅输出实际经过两条模拟
放大路径，辅路可能有独立非线性、建立过程和采样时刻；上述公式不能据此
视为物理恒等式。要验证这些机制，须分别产生第一级主、辅输出，使第二级
flash 观察辅路，而第二级 CDAC 继续观察主路。
