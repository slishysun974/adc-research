...
当前版本计算以下端点参考的累计码密度偏差：

$$
D_j^{(\mathrm{density})}=\sum_{i=0}^{j-1}\operatorname{DNL}_i,
\qquad j=0,1,\ldots,L.
$$

其中 $D_0^{(\mathrm{density})}=0$。当静态传输按码序单调时，
$D_j^{(\mathrm{density})}$ 等于第 $j$ 个码转换边界相对于理想边界的端点参考 INL，
不是码中心误差。若传输存在负向步或同一码对应多个不相邻的输入区间，这个累计量
仍可计算，但不再表示单一物理转换电平的 INL；报告应称其为“累计码密度偏差”，
并单独列出负向步或同一码对应多个输入区间的情况。既有数据字段 `transition_inl_lsb` 为兼容保留，
解释时须满足上述单调性条件。若 $H_i=0$，则码 $i$ 记为缺码，并有
$\operatorname{DNL}_i=-1$ LSB。报告同时给出 DNL、累计码密度偏差的最小值和
最大值，以及缺码列表；单调条件下可另将后者标注为转换电平 INL。不能用单一
最大绝对值代替正、负范围。
