# EXP-006：两级预设模拟传输与已知传输下的理想逆校正

> 状态：行为级通过；不代表 LMS 已收敛或定点 bit-true 已关闭  
> 架构配置：`gu_behavioral_v1`  
> 正式运行：`artifacts/runs/EXP_006/two_stage_pwl_truth_v1_20260915`

> 术语说明：`truth` 指实验预设的模拟传输，`oracle` 指由该已知传输反推的数字校正参考系数；两者均不是芯片已测参数。源码和运行字段沿用原名。数字 PWL 是校正器，预设模拟 PWL 只是一种可控测试条件。见[研究文档术语约定](../../TECHNICAL_WRITING_CONVENTIONS_V1.md)。

## 1. 结论

已建立两级相互独立的**预设分段一次模拟传输**、由这些已知传输反推的浮点理想逆校正
